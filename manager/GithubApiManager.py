...
class GithubApiManager:
    ...
    async def search_repos(self,text):
        async with aiohttp.ClientSession(headers={'Authorization': f'token {self.token}'}) as session:
            async with session.get(f'https://api.github.com/search/repositories?q={text}&per_page=100&sort=updated&order=desc') as response:
    ...
    async def load_repos(self):
        async with aiohttp.ClientSession(headers={'Authorization': f'token {self.token}'}) as session:
            async with session.get('https://api.github.com/user/repos?per_page=100&sort=created&direction=desc') as response:
                if response.status != 200:
                    return None
    ...
    async def edit_repo(self,repo_name,data):
        async with aiohttp.ClientSession(headers={'Authorization': f'token {self.token}'}) as session:
            async with session.patch(f'https://api.github.com/repos/{self.username}/{repo_name}' ,json=data) as response:
                if response.status != 200:
                    return None
                return response                   